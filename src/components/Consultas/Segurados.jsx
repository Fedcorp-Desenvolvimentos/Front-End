...
const ConsultaSegurado = () => {
  ...
  const performConsulta = async (page = 1) => {
    ...
    if (activeForm === "vida") {
      ...
      const isImoveisFormEmpty = imoveisFields.every(
    ...
    for (const key in parametroConsultaObj) {
      if (
        parametroConsultaObj[key] === null ||
        parametroConsultaObj[key] === ""
      ) {
          ...
          <>
            ...
            >
              <input
                ...
                disabled={loading}
                onFocus={() =>
                  formData.administradora.length > 0 && setShowSuggestions(true)
                }
                autoComplete="off"
